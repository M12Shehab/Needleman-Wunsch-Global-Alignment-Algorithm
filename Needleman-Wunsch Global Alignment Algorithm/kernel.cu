...
__global__ void ShihabKernal(char *Adata, char *Bdata, int slice, int z, int blen, int *NewH, int Increment, int Max)
{
	//int i = threadIdx.x;
...
int main(int argc, char * argv[])
{
	...
	for (int slice = 0; slice < 2 * alen - 1; ++slice)
	{
		...
		ShihabKernal << <ThreadsWarp, MemSize >> >(dev_a, dev_b, slice, z, alen, dev_H, Increment, size);
	}
